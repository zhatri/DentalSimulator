namespace TurnTheGameOn.Timer
{
    ...
    public class Timer : MonoBehaviour
    {
        ...
        [Tooltip("Timer will run as a clock and output UTC time. Requires useSystemTime to be enabled.")]
        public bool useUTC;
        ...
        void SetSystemTime()
        {
            if (useUTC)
            {
                gameTime = (double)DateTime.UtcNow.Hour + (double)DateTime.UtcNow.Minute + (double)DateTime.UtcNow.Second;
                second = (int)DateTime.UtcNow.Second;
        ...
        string FormatSeconds(double elapsed)
        {
            ...
            if (timerType == TimerType.CountUpInfinite)
            {
                if (useSystemTime)
                {
                    ...
                    {
                        gameTime = (double)DateTime.UtcNow.Hour + (double)DateTime.UtcNow.Minute + (double)DateTime.UtcNow.Second;
                        millisecond = (int)DateTime.UtcNow.Millisecond;
                        second = (int)DateTime.UtcNow.Second;
                        minute = (int)DateTime.UtcNow.Minute;
                        hour = (int)DateTime.UtcNow.Hour;
                        day = (int)DateTime.UtcNow.DayOfYear;
                    }
                    else
                    {
                        gameTime = (double)DateTime.Now.Hour + (double)DateTime.Now.Minute + (double)DateTime.Now.Second;
